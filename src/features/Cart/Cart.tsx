              ...
              <>
                    ...
                    <div key={idx} className="flex items-center space-x-4 border-b pb-2">
                      ...
                      <section className=" w-full">
                        ...
                        <p className="text-xs text-gray-500">
                          {/* {item.extras && item.extras.length > 0 ? "Con adiciones" : "Sin adiciones"} */}
                          <ul>
                            {item.extras && item.extras.length > 0 ? (
                              item.extras.map((extra, index) => (
                                <li key={index} className="text-xs text-gray-500 flex flex-row justify-between">
                ...
                <div className=" pt-4 mt-4">
                  ...
                  <button className="flex justify-between pl-12 pr-12  w-full bg-orange-500  font-semibold py-3 rounded-xl shadow-md mt-4">
                    ...
                    <p className="text-white">${cartItems.reduce((acc, item) => acc + item.totalPrice, 0).toFixed(2)}</p>
                  </button>
                </div>
              </>
            )}
